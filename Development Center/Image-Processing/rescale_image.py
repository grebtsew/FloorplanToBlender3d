import cv2
import os
import numpy as np

example_image_path = os.path.dirname(os.path.realpath(__file__))+"/../../Images/door.png"

from PIL import Image


scalefactor = 3 # downscale < 1 < upscale


def pil_rescale_image(image,factor ):

    width, height = c.size

    d = c.resize((width*factor,height*factor), resample=Image.BOX)

    return d

def cv2_rescale_image(image,factor):
    #percent by which the image is resized

    #calculate the 50 percent of original dimensions
    width = image.shape[1]*factor
    height = image.shape[0]*factor

    # dsize
    dsize = (width, height)

    # resize image
    return cv2.resize(image, dsize)

def pil_to_cv2(image):
    return cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR)

# Pil read
c = Image.open(example_image_path)

# for cv2
d = cv2_rescale_image(pil_to_cv2(c), scalefactor)

# for pil
d2 = pil_rescale_image(c, scalefactor)
d2 = pil_to_cv2(d2)

# Show result
cv2.imshow("origin", pil_to_cv2(c))
cv2.imshow("reshape-cv2", d)
cv2.imshow("reshape-pil", d2)
cv2.waitKey(0)

# Save to file
cv2.imwrite(os.path.dirname(os.path.realpath(__file__))+"/rescaled.png",pil_to_cv2(d2))